// In-process esbuild bundle backend. esbuild's Node launcher touches
// process.versions at module scope, so this file must stay out of the neutral
// module graph (cloud Workers): publish() loads it lazily only when a host did
// not inject a bundler. Import it directly only from Node-side code and tests.
import { build, version as esbuildVersion } from "esbuild";
import { Effect, Option, Schema } from "effect";

import { AppExecutorError } from "../executor/app-tool-executor";
import type { ToolchainRef } from "./descriptor";
import type { BundleBackend } from "./bundle";
