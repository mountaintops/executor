...
import { MICROSOFT_GRAPH_BASE_URL, microsoftGraphPreset } from "./presets";
...
export interface MicrosoftPluginExtension {
  readonly addGraph: (
  ...
  readonly updateGraph: (
    slug: string,
    input?: MicrosoftUpdateInput,
  ) => Effect.Effect<
    MicrosoftUpdateResult,
    | OpenApiParseError
    | OpenApiExtractionError
    | OpenApiOAuthError
    | IntegrationNotFoundError
    | StorageFailure
  >;
  readonly removeGraph: (slug: string) => Effect.Effect<void, StorageFailure>;
  readonly getIntegration: (slug: string) => Effect.Effect<Integration | null, StorageFailure>;
  readonly getConfig: (
    slug: string,
  ) => Effect.Effect<MicrosoftGraphIntegrationConfig | null, StorageFailure>;
  readonly configure: (
    slug: string,
    input: MicrosoftConfigureInput,
...
export const microsoftPlugin = definePlugin((options?: MicrosoftPluginOptions) => ({
  ...
  extension: (ctx): MicrosoftPluginExtension => {
