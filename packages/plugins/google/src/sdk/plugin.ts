...
export interface GooglePluginExtension {
  readonly addBundle: (
  ...
  readonly updateBundle: (
    ...
    | StorageFailure
  >;
  readonly removeBundle: (slug: string) => Effect.Effect<void, StorageFailure>;
...
export const googlePlugin = definePlugin((options?: GooglePluginOptions) => ({
  ...
  extension: (ctx): GooglePluginExtension => {
    ...
    return {
      ...
      getConfig: (slug: string) =>
        ctx.core.integrations
          ...
          .pipe(
            Effect.map((record) => (record ? decodeOpenApiIntegrationConfig(record.config) : null)),
          ),
