...
const formatDescription = (connectionEntries: readonly ConnectionInventoryEntry[]): string => {
  const lines: string[] = [
    ...
    '- File-returning tools may return `ToolFile` values: `{ _tag: "ToolFile", name?, mimeType, encoding: "base64", data, byteLength }`. To send the file to the MCP client or user, return the `ToolFile` unchanged, or include it unchanged inside your returned object. Executor will emit images as MCP image content, text files as decoded text, and other binaries as MCP resource content.',
    "- Some providers, including Gmail, return attachment bytes without a public URL. If you need to upload or send that file to another API from code, decode `ToolFile.data` from base64 and pass the bytes to that API's upload/file input instead of looking for a URL.",
