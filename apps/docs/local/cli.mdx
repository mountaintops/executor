...
| `executor daemon run`                 | Start the persistent local daemon             |
| `executor daemon status`              | Show daemon status                            |
| `executor daemon stop`                | Stop the daemon                               |
| `executor daemon restart`             | Restart the daemon                            |
| `executor mcp`                        | Start the MCP endpoint                        |
| `executor tools search "<query>"`     | Search tools by intent                        |
| `executor tools sources`              | List configured integrations + tool counts    |
| `executor tools describe <path>`      | Show a tool's TypeScript / JSON schema        |
| `executor call <path...> '{...}'`     | Invoke a tool by path segments                |
| `executor call <path...> --help`      | Browse namespaces / resources / methods       |
| `executor resume --execution-id <id>` | Resume a paused execution                     |
