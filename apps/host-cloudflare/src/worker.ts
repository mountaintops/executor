...
export default {
  fetch: async (request: Request, env: CloudflareEnv, ctx: ExecutionContext): Promise<Response> => {
    ...
    // Directly intercept all Salesforce OAuth, REST Proxy, MCP Proxy, Webhooks, OpenAPI Spec & Log routes
    if (url.pathname.includes("/sf") || url.pathname.includes("/oauth") || url.pathname.includes("/webhook") || url.pathname.includes("openapi") || url.pathname.includes("/logs")) {
